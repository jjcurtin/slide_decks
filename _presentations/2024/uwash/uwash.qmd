...
::: {.notes}
So when I introduced sensing earlier, I indicated that it must be feasible and acceptable to users over reasonably long periods of time so lets take a look at that for our three sensed signals.

Our participants completed approximately 80% of the 4x Daily EMAs over the three months period with only a modest drop in compliance over time.

Importantly, the completion rates for at least 1 EMA per day were even higher - at 95% 

And although I won't show these results today, we are now building models using only 1 EMA per day that can match the performance of the models I will show you with 4 EMA per day

And I also want to note that the NIDA funded project I mentioned earlier is collecting 1 EMA per day for a year from individuals with high rates of compliance.

So the bottom line is that it may be possible to sense EMA for the long periods we need for SUD with adequate compliance
:::
...
::: {.notes}
They also reported their subjective perceptions of the sensing methods.  Specifically, for each sensing method, we asked them to report

- if it interfered with their daily activities
- if they disliked providing it to us
- and if they would be willing to provide it for a year if it helped their recovery

In the figures to the right, I show histograms for these three questions with the scale oriented so that responses indicating higher acceptability are displayed to the right.

As you can see, most participants found EMA to be at least neutral or better

And I should note, in the context of this study, there were no clinical benefits provided to participants for engaging with our sensing system.  Its at least reasonable to expect that these sensing methods may be viewed more favorabily still by participants when used to help them with their recovery.
:::
...
Now let me turn to the more passive, but perhaps also more sensitive sensing methods we use.  Candidly, when we started this project, we weren't certain participants would provide these data to us, but it turns out they will.

To start, participants were not required to provide us with geolocation to participate in the study.  Instead, we provided a very modest monetary incentive, $10/month in the context of about $800 of total study payments.

Nonetheless, 100% of the participants opted-in to provide us with their location data.
:::
...
- [100% opted-in to provide geolocation]{style="color: gray"}
- [Very modest ($10/month) financial incentive]{style="color: gray"}
...
::: {.notes}
We also asked them if they disliked providing these data and if they were would be willing to provide it for a year.  We didnt ask the interference question because these data are passively collected with little to no burden on the user.

And as you can see, similar to EMA, most participants' evaluations of the acceptability of sensing geolocation was neutral or better.
:::

-----------------------------------------------------------------------------

## Sensing Feasibility/Acceptability: Communication Logs and Text Content
...
::: {.notes}
We did the same evaluations for our sensing of participants' communications.  We collected both the meta data or logs from their phones that contained the dates, times, and numbers of all incoming and outgoing calls and messages and we also collected the actual content of their text messages.  

And similar to geolocation, almost everyone was willing to provide these data to us with only a modest financial incentive
:::
...
- [150/151 opted-in to provide communication logs and text message content]{style="color: gray;"} 
- [Very modest ($10/month) financial incentive]{style="color: gray;"} 
...
::: {.notes}
We again asked the same "dislike" and "willingness to use for a year" questions, separately for the communication logs and text message content.

To the right, I am showing you their perceptions about sharing the logs were mostly neutral or better 
...
- [Perceptions about sharing logs were neutral or better for most]{style="color: gray;"} 
- And essentially same story for sharing text message content
::::
...
::: {.notes}
And the story is essentially the same about sharing the content of their text messages with us, with maybe a few more participants expressing some dislike.

Again, to be candid, we were pretty excited about these data because we believe they speak directly to the feasibility of using all three of this potentially powerful sensed signals for lapse prediction.

And I should also say that we've collected these same three sensed signals in our NIDA project in over 400 individuals with opioid use disorder and the short story there is that they too will give us these data, even though many of them are engaging in the use of a drug that has potential legal ramifications for them.
:::
...
But there are several reasons that we DO NOT want to predict dichoutomous class labels.
...
Second, these days, I think we have all come to understand that taking scores that are natively quantitative like probabilities are, and artificially dichotomoizing them results in substantial loss of information that is potentially valueable.
:::
...
## (Selective) Next Steps: Other Precision Mental Health Applications
