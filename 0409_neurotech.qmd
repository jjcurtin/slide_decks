...
As a first step, we took the model which predicted lapses in the next week and lagged it by different periods from one day up to one week.   For example, the two week lagged model predicts the probability of a lapse that will occur during a one week period but that period begins two weeks from now.  
...
And we are currently exploring other methods for predicting lapse probabilities further into the future that may perform better than this simple lagging approach and I'd be happy to talk more about those during the discussion period if anyone is intersted.
:::
