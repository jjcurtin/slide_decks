...
and an auROC of 1.0 indicates perfect performance 
...
But we can, and to be clear, we believe that all three of these models  have clinically implementable levels of performance such that we can use them to make meaningful decisions for individual participants at specific moments in time if we can embed these models within a recovery monitoring and support system.
:::
...
Do [NOT]{.uwred} provide model [output to clinicians]{.uwred}
...
Do [NOT]{.uwred} predict [class labels]{.uwred}  (lapse vs. no-lapse)
...
[DO]{.uwred} use [lapse probability]{.uwred}
...
- [These features can be used to personalize daily support recommendations]{.gray}
...
::: {.columns}
:::: {.column width="50%"}
- [Previous models only predict immediate future]{.gray}
- [Advanced warning needed for some types of supports]{.gray} 
