...
Personal sensing collects information from smartphones and other sensors to identify a person’s [thoughts, feelings, behaviors, and context]{.uwred}.
...
Sensing provides measurement that is/has:
...
[Sensing provides measurement that is/has:]{.gray}

- ["Real world" (in vivo)]{.gray}
- [Longitudinal]{.gray}
- [High temporal granularity]{.gray}
- [Feasible/acceptable for long term use]{.gray}
...
::: {.notes}
Given our sensing goals and requirements, we are focusing the majority of our effort at this point on feature engineering with three separate but complementary sets of raw signals that can be sensed easily on any smartphone. These are
...
The plot on the right shows feature names and their associated importance as indexed by mean absolute SHAP values across all the predictions made by the Next Hour Model. 

The bar width shows the relative importance of each feature for each model, globally across all participants and observations.  From this we see a few important characteristics of the NEXT HOUR model.   

First, all of EMA items affect predictions about lapse probability across observations  As you might expect, history of past lapses has a big influence on the probability of a future lapse.  But self reported abstinence efficacy, craving, emotional state, history of stress events, and other features from the EMA all make meaningful contributions to lapse probability in the next hour across observations.  
:::
-----------------------------------------------------------------------------
...
- Lapse day and Lapse hour are useful as expected
::::
...
::: {.notes}
We also see that we can use lapse day and lapse hour to make predictions of the lapse probability in the next hour.  Not surprisingly, people are more likely to lapse on weekends and during evening hours and the hour level model can use that information to improve its lapse predictions.  
...
- [Lapse day and Lapse hour are useful for day and hour level models as expected]{.gray}
...
But we should be careful to remember that we had limited racial and ethnicity diversity in the sample and this may have limited our ability to detect differences in lapse risk by those characteristics.
:::
...
cowplot::plot_grid(fig_fair_race_1, NULL,
                   fig_fair_income_1, fig_fair_sex_1)
```
...
cowplot::plot_grid(fig_fair_race_2, NULL,
                   fig_fair_income_2, fig_fair_sex_2)
```

::: {.notes}
And much more importantly, this next day prediction model appears much fairer with respect to its performance in all three of the subgroups where our original models were deficient.
...
## Next Steps: How to Yield Clinical Benefits 
...
- SHAP values can identify the most important risk features for a [specific individual each day]{.uwred}
- Day model can be used to identify each patients daily lapse risk, risk features, and personalized supports or activities to address those risks. 
- Morning messages could:w
...
Previously I showed you how we can use SHAP values across all observations to determine which features are globally important across all people and timepoints.   

We can also use those same SHAP values to understand which features contributed most strongly to any prediction for a specific person at a specific moment in time. This allows us to understand not only WHEN a lapse might occur but also WHY and potentially how best to intervene.

For example, we could deploy the next day model to get :::
...
## Clinical Uses: Advanced Warning 
