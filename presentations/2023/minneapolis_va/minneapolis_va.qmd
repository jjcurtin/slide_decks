...
<!--First slide title here before load data to avoid blank page after title page-->
...
::: {.column width="40%"}
<!--Lapse Probability by Ground Truth-->
```{r fig_hist_proabilites_threshold}
#| fig-height: 6
#| fig-width: 6
...
First, all of EMA items affect predictions about lapse probability globally across observations  And although not displayed here, all of these EMA items show meaningful variation across observations locally such that they each make important contributions to predictions in at least some instances.  

Second, demographics were not particularly important for prediction lapses, though I'll return later to acknowledged that we had limited race/ethnicity diversity in the sample.

And third, we can use lapse day and lapse hour to make predictions with the more temporally precise models.  Not surprisingly, people are more likely to lapse on weekends and during evening hours and the associated day and hour level models models can use that information to improve their lapse predictions.  
:::
...
```{r table_metrics_ppv_1}
# remove some data
metrics_subset <- metrics_all
metrics_subset[7, 2:4] <- NA
...
lr <- tibble(row_name = "Lapse Rate", Week = "25.4%", Day = "7.7%", Hour = "0.4%")
...
fig_hist_day <- preds_day |> 
  mutate(truth = if_else(truth == "no_lapse", "No lapse", "Lapse"),
         ...
         model = "week") |> 
  ggplot(data = _, aes(x = prob)) + 
   geom_histogram(bins = 15, fill = "green", col = "black", alpha = .4) +
   facet_wrap(~truth, nrow = 2, scales = "free_y") +
   xlab("Pr(Lapse)") +
   scale_y_continuous(labels = scales::comma)
...
```{r}
thresholds <- tibble(metric = c("Sensitivity", "Specificity", "", "PPV"),
                     thresh_1 = c("0.81", "0.86", "", "0.32"), 
                     thresh_2 = c("", "", "", ""))

thresholds |>
 kbl(format = "html", col.names = c("", "Thres = 0.50", "Thres = 0.90"),
      digits = 2,
      align = c("r", "r", "r"),
     linesep = "") |> 
  row_spec(row = 0, align = "r") |> 
  kable_styling(full_width = FALSE) |> 
  column_spec(2, color  = c("black", "black", "black", "red"), bold = TRUE) 
```
...
```{r fig_hist_probabilites_day_threshold}
#| fig-height: 6
#| fig-width: 6
...
thresholds |>
 kbl(format = "html", col.names = c("", "Thres = 0.50", "Thres = 0.90"),
  ...
  row_spec(row = 0, align = "r") |> 
  kable_styling(full_width = FALSE) |> 
  column_spec(3, color  = c("black", "black", "black", "red"), bold = TRUE)
```
...
![nida_logo.png](https://github.com/jjcurtin/lectures/blob/main/images/niaaa_logo.png?raw=true){.absolute bottom="10%" right="2%" width="auto" height="auto"}\ 
