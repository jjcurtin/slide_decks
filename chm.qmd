...
- Recruited ~ 300 patients in recovery from Opioid Use Disorder 
...
::: {.notes}
For example, we were awarded a grant from NIDA to collect a more racially diverse sample using nationwide recruiting techniques.  

This sample also included much needed geographic diversity because the factors that predict lapse in urban settings may be different from those that predict lapse in rural settings.
:::
...
We are in the midst of finalizing model training using these data but I'll give you a peek at performance for models using EMA features similar to the AUD models models I showed you previously.   We are particularly excited by these models because we restricted the EMAs to only 1 per day but we are still seeing the best performance we have seen to date when predicting lapses in the next day.
...
Data collection begins in ~ 1 month
