...
```{r}
make_barplot <- function(d, labels, title){
  d |>
    ...
    ggplot(aes(x = value)) +
      geom_bar() +
      geom_vline(aes(xintercept = "Undecided"), linetype = "dashed", linewidth = .4, color = "black") +
    xlab("Rating") +
    ggtitle(title) +
    theme(
...
plot_interfere  <- sr |> 
  ...
  make_barplot(labels = c("Strongly agree", "Agree", "Undecided", 
...
plot_willing <- sr |> 
  ...
  make_barplot(labels = c("Strongly disagree", "Disagree", "Undecided", 
...
#| fig-height: 8
#| fig-width: 6
cowplot::plot_grid(plot_interfere, plot_dislike, plot_willing, 
                   nrow = 3)
```
